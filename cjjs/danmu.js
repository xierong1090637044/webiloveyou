	...
	$.fn.barrage = function(opt){
		...
		M.barrageBox = $('<div id="barrage" class ="barrage"></div>'); //存所有弹幕的盒子
		M.timer = null;
		var createView = function(){
			var randomIndex = Math.floor(Math.random() * M.bgColors.length);
			var ele = $('<div class="overflow-text" style="height:0;opacity:0;text-align:'+settings.direction.split(/\s+/)[1]+';float:'+settings.direction.split(/\s+/)[1]+';background-color:'+M.bgColors[randomIndex]+'";>'+Obj.data[0].text+'</div>');
